...
int BoundedPocketStandHarvester::EVENT_GROUP_SELECTION_REGIME_70_clear_cut(int i, int j){
	...
	int TmpNum = 0;//add by houbin 11/1/2017

	LANDUNIT *l;

	l = pCoresites->locateLanduPt(i,j);

	SITE * siteptr = (*pCoresites) (i, i);//add by houbin chen 4/28/2017
		...
		if(pCoresites->flag_cut_GROUP_CUT[k-1]){
			...
			for(m=1;m<=(*pCoresites)(i,j)->specAtt(k)->longevity/pCoresites->TimeStep;m++){
				...
				if((*pCoresites)(i,j)->SpecieIndex(k)->getTreeNum(m,k)>0){
					...
					TmpNum = (*pCoresites)(i, j)->SpecieIndex(k)->getTreeNum(m, k);					//add by houbin 11/1/2017
					...
					TmpBio = exp(pCoresites->GetBiomassData(siteptr->specAtt(k)->BioMassCoef, 1) + pCoresites->GetBiomassData(siteptr->specAtt(k)->BioMassCoef, 2)*log(pCoresites->GetGrowthRates(k, m, l->ltID)))*(*pCoresites)(i, j)->SpecieIndex(k)->getTreeNum(m, k) / 1000.00;
					...
					{
						pHarvestsites->AddMoreValueHarvestBA_spec(i, j, k - 1, TmpBasalAreaS);	//add by houbin 11/1/2017
						pHarvestsites->AddMoreValueHarvestBio_spec(i, j, k - 1, TmpBio);	//add by houbin 11/1/2017
						pHarvestsites->AddMoreValueHarvestNum_spec(i, j, k - 1, TmpNum);	//add by houbin 11/1/2017

						for (int agerange_index = 1; agerange_index < pCoresites->GetOutputAgeRangeModuleFlagArray(k - 1, COUNT); agerange_index++)		//add by houbin 11/1/2017
						{
							if (pCoresites->GetOutputAgeRangeModuleFlagArray(k - 1, agerange_index, 0) < m*pCoresites->TimeStep && m*pCoresites->TimeStep <= pCoresites->GetOutputAgeRangeModuleFlagArray(k - 1, agerange_index, 1))
							{
								pHarvestsites->AddMorevalueHarvestBA_spec_interval(i, j, k - 1, agerange_index - 1, TmpBasalAreaS);
								pHarvestsites->AddMorevalueHarvestBIO_spec_interval(i, j, k - 1, agerange_index - 1, TmpBio);
								pHarvestsites->AddMorevalueHarvestNum_spec_interval(i, j, k - 1, agerange_index - 1, TmpNum);
							}
						}

						for (int ii = 0; ii < pHarvestsites->inter_number; ii++)
						{
							if (m*pCoresites->TimeStep <= pHarvestsites->interval[ii * 2 + 1] && m*pCoresites->TimeStep > pHarvestsites->interval[ii * 2])
							{
								pHarvestsites->AddMoreValueHarvestBA_interval(i, j, ii, TmpBasalAreaS);
								//pHarvestsites->AddMorevalueHarvestBIO_spec_interval(i, j, k - 1, ii, TmpBio);
							}
						}
					}
