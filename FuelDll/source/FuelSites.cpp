...
//houbin 1/3/2017
int FuelSites::setDTN(int* DTN, int row, int col){
	//this function will set the dead tree number into PDP
	//cout << row <<" "<<col << endl;
	//printf("%d %d\t",row,col);
	//printf("%d\n",sizeof(DTN) / sizeof(DTN[0]));
	//printf("%d\n", sizeof(pPDP->cDeadTreeNum[row][col]) / sizeof(pPDP->cDeadTreeNum[row][col][0]));
	//getchar();
	for (int iii = 0; iii < pPDP->specnum; iii++)
	{
		pPDP->cDeadTreeNum[row - 1][col - 1][iii] = DTN[iii];
	}
	//(pPDP->cDeadTreeNum)[row-1][col-1] = DTN;
	return 0;
}
int FuelSites::setDTNBIO(float* DTNBIO, int row, int col){
	//this function will set the dead tree number biomass into PDP
	for (int iii = 0; iii < pPDP->specnum; iii++)
	{
		pPDP->cDeadTreeBio[row - 1][col - 1][iii] = DTNBIO[iii];
	}
	//(pPDP->cDeadTreeBio)[row - 1][col - 1] = DTNBIO;
	for (int iii = 0; iii < pPDP->specnum; iii++)
	{
		if ((pPDP->cDeadTreeBio[row - 1][col - 1][iii] == 0.0f && pPDP->cDeadTreeNum[row - 1][col - 1][iii] != 0) || (pPDP->cDeadTreeBio[row - 1][col - 1][iii] != 0.0f && pPDP->cDeadTreeNum[row - 1][col - 1][iii] == 0))
		{
			printf("\n(%d,%d,%d)dtn and dtnbio different3 %d %f\n", row - 1, col - 1, iii, (pPDP->cDeadTreeNum)[row - 1][col - 1][iii], (pPDP->cDeadTreeBio)[row - 1][col - 1][iii]);
		}


		if (row == 166 && col == 149 && iii == 0)
		{
			//printf("captured: %d %f\n", (pPDP->cDeadTreeNum)[row - 1][col - 1][iii], (pPDP->cDeadTreeBio)[row - 1][col - 1][iii]);
		}
		if (row == 166 && col == 152 && iii == 0)
		{
			//printf("captured: %d %f\n", (pPDP->cDeadTreeNum)[row - 1][col - 1][iii], (pPDP->cDeadTreeBio)[row - 1][col - 1][iii]);
		}
...
int FuelSites::setDTNBA(float* DTNBA, int row, int col){
	for (int iii = 0; iii < pPDP->specnum; iii++)
	{
		pPDP->cDeadTreeBA[row - 1][col - 1][iii] = DTNBA[iii];
	}
	return 0;
}
int FuelSites::getDTN(int row, int col, int specIndex){
	int DTN = 0;
	if (row<0 || row>rows || col<0 || col>cols)
		return -1;
	DTN = pPDP->cDeadTreeNum[row][col][specIndex];
	if (DTN < 0)
	DTN = 0;
	return DTN;
}
...
int FuelSites::setFFLoad(int ffload, int row, int col) {
	...
	if ((row == 159 && col == 158) || (row == 161 && col == 159) || (row == 143 && col == 149) || (row == 144 && col == 149) || (row == 144 && col == 150) || (row == 157 && col == 149))
	{
		//printf("hold");
	}
