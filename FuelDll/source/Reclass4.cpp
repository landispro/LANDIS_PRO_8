...
void FuelReclass_DTNBIO(MAP8 &m, int specIndex, FILE * fueloutfile) {
	...
	sprintf(str, "Dead tree biomass class representation");
	...
	for (i = snr; i >= 1; i--){
		...
		for (j = 1; j <= snc; j++) {
			...
			int dtn = fuelsites.getDTN(i,j,)
			}*/
			int x = ((i - 1)*snc) + j - 1;
			assert(x <= 2147483647 && x >= -2147483647);
			if (pPDP->cDeadTreeBio[i - 1][j - 1][specIndex] < 0){
				//m(i, j) = 0;
				m.data_bio[x]=0;
				//m.data_bio[];
			}
			else{
				m.data_bio[x] = pPDP->cDeadTreeBio[i - 1][j - 1][specIndex];
			}

			if ((pPDP->cDeadTreeBio[i - 1][j - 1][specIndex] == 0.0f && pPDP->cDeadTreeNum[i - 1][j - 1][specIndex] != 0) || (pPDP->cDeadTreeNum[i - 1][j - 1][specIndex] == 0 && pPDP->cDeadTreeBio[i - 1][j - 1][specIndex] != 0.0f))
			{
				printf("\n(%d,%d,%d)dtn and dtnbio different4 %d %f", i,j,specIndex, pPDP->cDeadTreeNum[i - 1][j - 1][specIndex], pPDP->cDeadTreeBio[i - 1][j - 1][specIndex]);
			}


			/*if ((m(i, j) == 0 && m.data_bio[x] != (float)0) || (m(i, j) != 0 && m.data_bio[x] == (float)0))
			{
				printf("\n(%d,%d,%d)dtn and dtnbio different5 %d %f", i,j,specIndex, m(i, j), m.data_bio[x]);
			}*/
...
void FuelReclass_DTNBA(MAP8 &m, int specIndex, FILE * fueloutfile) {
	...
	for (i = snr; i >= 1; i--){
		...
		for (j = 1; j <= snc; j++) {
			...
			int x = ((i - 1)*snc) + j - 1;
			assert(x <= 2147483647 && x >= -2147483647);
			if (pPDP->cDeadTreeBA[i - 1][j - 1][specIndex] < 0){
				//m(i, j) = 0;
				m.data_bio[x] = 0;
				//m.data_bio[];
			}
			else{
				m.data_bio[x] = pPDP->cDeadTreeBA[i - 1][j - 1][specIndex];
			}

			if ((pPDP->cDeadTreeBA[i - 1][j - 1][specIndex] == 0.0f && pPDP->cDeadTreeNum[i - 1][j - 1][specIndex] != 0) || (pPDP->cDeadTreeNum[i - 1][j - 1][specIndex] == 0 && pPDP->cDeadTreeBA[i - 1][j - 1][specIndex] != 0.0f))
			{
				printf("\n(%d,%d,%d)dtn and dtnbio different4 %d %f", i, j, specIndex, pPDP->cDeadTreeNum[i - 1][j - 1][specIndex], pPDP->cDeadTreeBA[i - 1][j - 1][specIndex]);
			}
...
void FuelReclass_DTN(MAP8 &m, int specIndex, FILE * fueloutfile) {
	...
	sprintf(str, "Dead tree number class representation");
	...
	for (i = snr; i >= 1; i--){
		...
		for (j = 1; j <= snc; j++) {
			...
			if (pPDP->cDeadTreeNum[i-1][j-1][specIndex] < 0){
				m(i, j) = 0;
			}
			else{
				m(i, j) = pPDP->cDeadTreeNum[i-1][j-1][specIndex];
			}
			/*if (pPDP->cDeadTreeNum[i - 1][j - 1][specIndex] != 0)
			{
				printf("fuelburnd%d(%d,%d)%d ", specIndex, i-1, j-1, pPDP->cDeadTreeNum[i - 1][j - 1][specIndex]);
			}*/
...
void FuelReclass_DTN_spec_agerange(MAP8 &m, int spec_index, int agerange_index, FILE * fueloutfile) {	//add by houbin 10/26/2017
	...
	for (i = snr; i >= 1; i--){
		...
		for (j = 1; j <= snc; j++) {
			...
			if (pPDP->cDeadTreeNum_spec_agerange[i - 1][j - 1][(agerange_index - 1)*pPDP->specnum + spec_index] < 0){
				m(i, j) = 0;
			}
			else{
				m(i, j) = pPDP->cDeadTreeNum_spec_agerange[i - 1][j - 1][(agerange_index - 1)*pPDP->specnum + spec_index];
	...
	for (i = 1; i <= snr; i++){
		...
		for (j = 1; j <= snc; j++) {
			...
			if (m(i, j) != 0)
			{
				//printf("%d(%d,%d)%d ",interval_index,i-1,j-1, m(i, j));
			}
...
void FuelReclass_DTNBA_spec_agerange(MAP8 &m, int spec_index, int agerange_index, FILE * fueloutfile) {	//add by houbin 10/26/2017
	...
	for (i = snr; i >= 1; i--){
		...
		for (j = 1; j <= snc; j++) {
			...
			if (pPDP->cDeadTreeBA_spec_agerange[i - 1][j - 1][(agerange_index - 1)*pPDP->specnum + spec_index] < 0){
				m(i, j) = 0;
			}
			else{
				m(i, j) = pPDP->cDeadTreeBA_spec_agerange[i - 1][j - 1][(agerange_index - 1)*pPDP->specnum + spec_index];
...
void FuelReclass_DTNBIO_spec_agerange(MAP8 &m, int spec_index, int agerange_index, FILE * fueloutfile) {	//add by houbin 10/26/2017
	...
	for (i = snr; i >= 1; i--){
		...
		for (j = 1; j <= snc; j++) {
			...
			if (pPDP->cDeadTreeBio_spec_agerange[i - 1][j - 1][(agerange_index - 1)*pPDP->specnum + spec_index] < 0){
				m(i, j) = 0;
			}
			else{
				m(i, j) = pPDP->cDeadTreeBio_spec_agerange[i - 1][j - 1][(agerange_index - 1)*pPDP->specnum + spec_index];
...
//////////////////////////////////////////////////////
/*void FuelReclass_DTN_age_interval(MAP8 &m, int interval_index, FILE * fueloutfile) {
	...
	for (i = snr; i >= 1; i--){
		...
		for (j = 1; j <= snc; j++) {
			...
			if (pPDP->cDeadTreeNum_ageinterval[i - 1][j - 1][interval_index] < 0){
				m(i, j) = 0;
			}
			else{
				m(i, j) = pPDP->cDeadTreeNum_ageinterval[i - 1][j - 1][interval_index];
		...
		for (j=1;j<=pfuelcoresites->numColumns();j++) {
			...
			{}
				...
				*/
				...
				fireProbility = 1 - exp((float)-1 * frand1());
